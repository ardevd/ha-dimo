...
async def test_dimo_client_get_vehicle_makes(mocker):
    ...
    dimo_mock = Mock()
    dimo_mock.device_definitions.list_device_makes = AsyncMock(
...
@pytest.mark.asyncio
async def test_auth_token_caching(mocker):
...
@pytest.mark.asyncio
async def test_auth_get_token_calls_get_auth_when_token_is_none(mocker):
...
@pytest.mark.asyncio
async def test_auth_get_privileged_token(mocker):
    fake_privileged_token = "privileged_token_1234"
    vehicle_token_id = "vehicle_token_id_1234"

    # Mock DIMO instance and token_exchange
    dimo_mock = Mock()
    dimo_mock.token_exchange.exchange = AsyncMock(return_value=fake_privileged_token)

    auth = Auth("client_id", "domain", "private_key", dimo=dimo_mock)
    auth.token = "current_token"

    # Test privileged token retrieval
    privileged_token = await auth.get_privileged_token(vehicle_token_id)
