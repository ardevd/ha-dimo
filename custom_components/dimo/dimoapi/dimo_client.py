...
from .queries import (CUSTOM_SIGNAL_FRAGMENTS, GET_ALL_VEHICLES_QUERY,
                      GET_LATEST_SIGNALS_QUERY, GET_MULTIPLE_VEHICLE_REWARDS_QUERY,
                      GET_VEHICLE_REWARDS_QUERY, VEHICLE_REWARD_FRAGMENT)
...
class DimoClient:
    ...
    def get_rewards_for_vehicles(self, token_ids: List[str]):
        """Get total token rewards for multiple vehicles in a single query"""
        if not token_ids:
            return {}

        # Build vehicle reward fragments
        vehicles_query = "\n".join(
            VEHICLE_REWARD_FRAGMENT.format(idx=i, token_id=token_id)
            for i, token_id in enumerate(token_ids, start=1)
        )

        query = GET_MULTIPLE_VEHICLE_REWARDS_QUERY.format(vehicles=vehicles_query)
        return self.dimo.identity.query(query)
